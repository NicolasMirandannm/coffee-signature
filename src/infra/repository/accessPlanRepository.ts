...
export default class AccessPlanRepository implements IAccessPlanRepository {
    ...
    private async findReceiver(receiverSchema: ReceiverEntity, cpf: string): Promise<ReceiverEntity> {
    ...
    async findAll(): Promise<Array<AccessPlan>> {
        const accessPlanDocument: AccessPlanEntity[] = await this.accessPlanModel.find().populate('receiver');

        return accessPlanDocument.map((accessPlan) => {
            return this.mapper.toDomain({accessPlanSchema: accessPlan, receiverSchema: accessPlan.receiver})
        })
    }
    ...
    async findById(id: string): Promise<AccessPlan> {
        ...
        const accessPlan: AccessPlanEntity = await this.accessPlanModel.findById(new Types.ObjectId(id)).populate('receiver');
        return this.mapper.toDomain({accessPlanSchema: accessPlan, receiverSchema: accessPlan.receiver});
    }

    async findByClientName(name: string): Promise<AccessPlan> {
        InfraException.whenParameterIsNull(name, 'could not find by undefined name.')

        const accessPlan: AccessPlanSchemaDocument = await this.accessPlanModel.findOne({name}).populate('receiver');
        if (accessPlan == null)
            return null;
    ...
    async update(accessPlan: AccessPlan): Promise<void> {
        const { accessPlanSchema, receiverSchema } = this.mapper.toPersistence(accessPlan);

        await this.accessPlanModel.findByIdAndUpdate(accessPlanSchema._id, accessPlanSchema);
        await this.receiverModel.findByIdAndUpdate(receiverSchema._id, receiverSchema);
    }

    async delete(id: string): Promise<void> {
        await this.accessPlanModel.findByIdAndDelete(new Types.ObjectId(id)).orFail(new InfraException('accessPlan does not exists!'));
