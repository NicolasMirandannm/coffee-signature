...
export default class UpdateSignatureService implements IUpdateSignature {
    ...
    async execute(id: string, signatureUpdateDto: SignatureUpdateDto): Promise<Signature> {
        const signature: Signature = await this.signatureRepository.findById(id);
        ApplicationException.whenParameterIsNull(signature, 'signature does not exists!');
