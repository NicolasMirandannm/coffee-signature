...
export default class AccessPlanRestController {
    ...
    @Patch('update/:id')
    update(@Param('id') id: string, @Body() accessPlanDto: AccessPlanDto): Promise<AccessPlan> {
         return this.updateAccessPlanService.execute(id, accessPlanDto);
    }

    @Delete('delete/:id')
    async delete(@Param('id') id: string): Promise<string> {
        return await this.deleteAccessPlanService.execute(id) ? 'success on delete!' : 'fail on delete!'
    }
}