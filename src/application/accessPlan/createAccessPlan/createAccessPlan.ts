...
export default class CreateAccessPlan implements ICreateAccessPlan {
  ...
  public async execute(accessPlanDto: AccessPlanDto): Promise<void> {
    ApplicationException.whenParameterIsNull(
      accessPlanDto,
      'could not create a accessPlan without parameter.',
    );
    ApplicationException.whenAnyPropInParameterIsNull(accessPlanDto);

    const accessPlanExists = await this.accessPlanRepository.findByName(
      accessPlanDto.planName,
    );
    if (accessPlanExists)
      throw new ApplicationException(
        `already exists access plan for label '${
          accessPlanExists.getProps().planName
        }'`,
      );
